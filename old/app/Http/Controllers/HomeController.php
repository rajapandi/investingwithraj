...
class HomeController extends Controller{
    ...
    public function getAllPosition(Request $request){
        $preRMS="";$jsonData="";$GetPosition="";$prePositions="";
        $net=0;$availablecash=0;$availableintradaypayin=0;$availablelimitmargin=0;$m2munrealized=0;$m2mrealized=0;
        $utilisedturnover=0;$utiliseddebits=0;$utilisedspan=0;
        $data=[];
        $GetHolding = array();
        $ta = TradingAccount::all();
        // $ta = TradingAccount::where('login_id', 'D179981')->get();
        if($ta){
            foreach($ta as $allta){
                // echo $allta->login_id;
                if($allta->stock_brocker=="Angel"){
    ...
    public function holding(Request $request){
        ...
        foreach($GetHolding as $value) {
            ...
            foreach($value['data'] as $object){
                ...
                $taskObject->totalInvestment = number_format((float)$object['quantity']*$object['averageprice'], 2, '.', '');
        ...
        if(isset($_GET['symbol'])){
            $ID = $_GET['symbol'];
            $json = array_filter($json, function ($var) use ($ID) {
                return ($var['tradingsymbol'] == $ID);
            });
        }
    ...
    public function portfolio(Request $request){
        ...
        if(isset($_GET['platform'])){
            if($_GET['platform']=="All"){
                ...
                $ta = TradingAccount::where('stock_brocker', $_GET['platform'])->get();
            }
        }else if(isset($_GET['account']) && $_GET['account']!="" && $_GET['account']!=null){
            $ta = TradingAccount::where('login_id', $_GET['account'])->get();
        }else{
            $ta = TradingAccount::all();
        }
        ...
        if(isset($_GET['search']) && !isset($_GET['sortby']) && $_GET['search']!=""){
            ...
            $property = 'loginId';
            $json = array_filter($json, function($country) use ($property, $search) {
                return strpos($country['loginId'], $_GET['search']) !== false;
            });
