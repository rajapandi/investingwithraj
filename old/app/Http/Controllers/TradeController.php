...
class TradeController extends Controller{
    ...
    public function searchTradeSymbol(Request $request){
        $sysmbolAngel = 'symbol:angel';
        $str = $request->key;
        $valuestr = "";
        if(unserialize(Redis::get($sysmbolAngel))!=null){
            $jsonitem = json_decode(unserialize(Redis::get($sysmbolAngel)));
        }else{
            ...
            $jsonData = file_get_contents("https://margincalculator.angelbroking.com/OpenAPI_File/files/OpenAPIScripMaster.json");
            Redis::set($sysmbolAngel, serialize($jsonData));
            $jsonitem = json_decode(unserialize(Redis::get($sysmbolAngel)));
        }
        foreach ($jsonitem as $friend) {
            if(preg_match("/$str/i", $friend->symbol, $matches, PREG_OFFSET_CAPTURE)==1){
                $valuestr = $friend->symbol;
                echo '<li onclick="getAddDataOnSerachBox(\''.$valuestr.'\')" style="cursor:pointer;">'.$friend->symbol.'</li>';
            }
        }
    }
