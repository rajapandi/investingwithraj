<div id="popupmarketDepth">
  <input type="hidden" name="symbol" id="symbol" value="{{$_GET['symbol']}}">
  <input type="hidden" name="exchange" id="exchange" value="{{$_GET['exchange']}}">
<?php
$sysmbol = $_GET['symbol'];
...
<script>
  $(document).ready(function() {
    ...
    console.log(time);
    if(time<"15:30:00" && time>"9:30:00"){
      // $('#popupmarketDepth').html('');
      var exchange = $('#exchange').val();
      var symbol = $('#symbol').val();
      setInterval(function() {
        $.get('/jqajax/popup-mrket-depth?symbol='+symbol+'&exchange='+exchange, {}, function(res){
          // $('#popupmarketDepth').html('');
          $('#popupmarketDepth').html(res);
        });
      }, 3000);
    }
});
